...
-- Create helper function in PUBLIC schema (auth schema is protected)
...
DROP POLICY IF EXISTS "View drafts" ON post_drafts;
DROP POLICY IF EXISTS "Create drafts" ON post_drafts;
DROP POLICY IF EXISTS "Update drafts" ON post_drafts;
DROP POLICY IF EXISTS "Delete drafts" ON post_drafts;
...
CREATE POLICY "View drafts"
  ...
  USING (
    auth.uid() = user_id
    OR (workspace_id IS NOT NULL AND public.user_is_workspace_member(workspace_id))
  );
